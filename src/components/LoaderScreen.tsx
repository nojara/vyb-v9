      ...
      <div className="relative w-full h-full" style={{ backgroundColor: '#000' }}>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 text-center w-full px-5 md:px-6">
          <motion.div
            ...
            <h1 className="type-display mb-6 md:mb-8 tracking-[-0.08em] text-[clamp(2.2rem,10vw,7rem)]" style={{ color: '#fff' }}>
              {formatText(lang === 'ar' ? 'جلسات VYB الحيّة' : 'VYB SESSIONS')}
            </h1>
            <p className="type-subtitle italic max-w-2xl mx-auto text-[clamp(0.85rem,3.5vw,1.5rem)]" style={{ color: 'rgba(255,255,255,0.6)' }}>
              {formatText(lang === 'ar' ? 'صوت الطاقة الإقليمية. مبادرة هندسة ثقافية من موبايلي.' : 'The Sound of Regional Energy. A Cultural Engineering Initiative by Mobily.')}
            </p>
          </motion.div>
        </div>

        <div className="absolute bottom-16 md:bottom-20 left-1/2 -translate-x-1/2 z-20" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
          <motion.button
            ...
            <span className="relative z-10">{lang === 'ar' ? 'ابدأ التجربة' : 'START EXPERIENCE'}</span>
            <div className="absolute inset-0 translate-y-full group-hover:translate-y-0 transition-transform duration-500" style={{ backgroundColor: '#E42217' }} />
          </motion.button>
        </div>

        <div className="absolute bottom-4 md:bottom-8 left-4 md:left-12 z-10 flex gap-4 md:gap-8 items-center opacity-30" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
